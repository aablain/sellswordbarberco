...
export default class Booking extends Component<Props, State> {
    ...
    getID() {
        const { bookedAppointment: bkdapp } = this.state
        const freshAppointment = this.props.appointments.find(app => (app.starthour === bkdapp.starthour && app.startminute === bkdapp.startminute && app.barber === bkdapp.barber && app.phone === bkdapp.phone));
        return freshAppointment.key;
    }
    ...
    submitAppointment() {
        ...
        if (gotEverything === true && everythingIsRight === true) {
            ...
            this.setState({ bookedAppointment: payload });
        }
    }
